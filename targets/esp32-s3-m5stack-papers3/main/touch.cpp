...
constexpr char kTag[] = "papers3_touch";
...
public:
	...
	bool begin()
	{
		...
		// Touch is REAL on this board: the GT911 sits on I2C SDA 41 / SCL 42 and
		// answers at 0x5D — confirmed by upstream PaperBoy running on this exact
		// unit ("GT911 found at I2C 0x5D"). The LilyGo target this file came from
		// deferred touch because its bus was unknown; that does not apply here,
		// and on a device with no user button touch is the only input.
		auto bus = gea::platform::i2c::Bus::primary();
		// Fail honestly, as the M5Paper target does. This board has no user
		// button, so touch is the ONLY input — reporting success without it
		// would leave a device that renders but can never be driven.
		...
		xTaskCreatePinnedToCore(touchTaskEntry, "papers3_touch", 3072, this, 7, &touchTask_, 0);
		return true;
	}

	void poll(int nowMs)
	{
		gea::platform::papers3::buttons::poll(nowMs);
