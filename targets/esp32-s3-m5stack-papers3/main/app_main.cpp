...
constexpr char kTag[] = "gea_papers3";
...
{
	// Unlike the M5Paper there is no battery soft-latch to assert here: power
	// on this board is owned by a PMS150G companion MCU, and the two panel
	// rails (PWR_EN / BST_EN) are raised inside the display engine itself
	// (papers3_epd_power_on), so display init owns panel power sequencing.
	gea::platform::papers3::buttons::init();
