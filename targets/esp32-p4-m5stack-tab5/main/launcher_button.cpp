...
constexpr gpio_num_t kGpio = GPIO_NUM_35;
...
public:
	...
	void start()
	{
		...
		// Resetting first forces the BOOT pad back to a clean GPIO matrix entry
		// in case a prior boot stage left alternate routing behind.
		...
		if (err != ESP_OK) {
			ESP_LOGE(kTag, "Failed to configure BOOT launcher button on GPIO%d: %s", static_cast<int>(kGpio), esp_err_to_name(err));
			return;
		}
