/*
 * Repo-local FatFs configuration override for the ESP32-P4 camera target.
 *
 * This is a verbatim copy of ESP-IDF v6.0.1's components/fatfs/src/ffconf.h with
 * ONE change: FF_FS_EXFAT is set to 1 so the board can mount exFAT microSD cards
 * (IDF exposes no Kconfig for exFAT). It is force-included into the fatfs component
 * (see main/CMakeLists.txt: `-include .../fatfs_ffconf.h`). Because the file opens
 * with `#ifndef _FFCONF_DEFINED`, defining that guard here makes IDF's own ffconf.h
 * a no-op, so this config wins WITHOUT modifying the ESP-IDF tree.
 *
 * Everything else is still driven from sdkconfig (LFN, sector size, code page,
 * volume label, ...). If ESP-IDF is upgraded, re-sync this file from the new
 * components/fatfs/src/ffconf.h and re-apply the FF_FS_EXFAT=1 change.
 */
...
#define FF_FS_EXFAT		1	/* gea: 1 (was 0 in IDF) — mount exFAT microSD cards */
