...
namespace gea::platform::esp32_p4_m5stack_tab5::camera {
...
// The SC202CS image is 90deg CCW relative to the Tab5 glass when written into
// the physical panel. Correct it with the P4 PPA (3 CCW turns == 1 CW turn).
constexpr int kPreviewRotateCcw = 3;
constexpr int kIspBrightnessDefault = 0;
constexpr int kIspBrightnessBright = 32;
constexpr int kIspBrightnessDark = -32;
...
public:
	...
	{
		if (index != 0) return {};
		return {"mipi-csi-sc202cs-center", gea::platform::camera::Facing::Front, kDefaultWidth, kDefaultHeight};
	}
	...
	{
		// The Tab5 camera has no optical/sensor zoom, so this is a digital zoom
		// applied in the preview scaler.
	...
	// SC202CS does not expose the AE target-level control used by OV5647. Keep
	// sensor AE running and map bias-only app states to ISP brightness instead.
	void setExposure(const std::string &mode, double bias, double iso, double durationMs)
	{
		(void)iso;
		(void)durationMs;
		int brightness = kIspBrightnessDefault;
		if (mode != "auto" && bias > 0.0) brightness = kIspBrightnessBright;
		else if (mode != "auto" && bias < 0.0) brightness = kIspBrightnessDark;
		setIspIntControl(V4L2_CID_BRIGHTNESS, brightness);
	}
	...
	// The Tab5 camera module has no controllable LED, so torch is a no-op here;
	// the hook exists so the unified API stays uniform.
...
private:
	...
	bool initializeVideo()
	{
		...
		if (!gea::platform::tab5::setCameraEnabled(true)) {
			ESP_LOGE(kTag, "camera power enable failed");
			return false;
		}
		...
		csiConfig.reset_pin = GPIO_NUM_NC;
		csiConfig.pwdn_pin = GPIO_NUM_NC;
	...
	bool queryAndConfigureFormat(int preferredWidth, int preferredHeight)
	{
		...
		// at esp_video init via CONFIG_CAMERA_SC202CS_MIPI_*; the active sensor mode
		// is selected there, not here).
	...
	// Mount the microSD card at /sdcard (SDMMC slot 1, 4-bit). Tab5 ESP32-P4
	...
	bool ensureIspDevice()
	{
		if (ispFd_ >= 0) return true;
		ispFd_ = ::open(ESP_VIDEO_ISP1_DEVICE_NAME, O_RDWR);
		if (ispFd_ < 0) {
			ESP_LOGW(kTag, "open %s failed: errno=%d", ESP_VIDEO_ISP1_DEVICE_NAME, errno);
			return false;
		}
		return true;
	}

	bool setIspIntControl(std::uint32_t id, int value)
	{
		if (!ensureIspDevice()) return false;
		v4l2_query_ext_ctrl query{};
		query.id = id;
		if (ioctl(ispFd_, VIDIOC_QUERY_EXT_CTRL, &query) == 0) {
			value = std::min<int>(static_cast<int>(query.maximum), std::max<int>(static_cast<int>(query.minimum), value));
		}
		v4l2_ext_control ctrl{};
		ctrl.id = id;
		ctrl.value = value;
		...
		ctrls.count = 1;
		ctrls.controls = &ctrl;
		if (ioctl(ispFd_, VIDIOC_S_EXT_CTRLS, &ctrls) != 0) {
			ESP_LOGW(kTag, "ISP S_EXT_CTRLS 0x%" PRIx32 "=%d failed: errno=%d", id, value, errno);
			return false;
		}
		return true;
	}
	...
	{
		setExtControl(V4L2_CID_VFLIP, 0);
		setExtControl(V4L2_CID_HFLIP, mirror_ ? 1 : 0);
	}
	...
	// while scaling. Fit + crop math run on the rotated source dimensions, and
	// each pixel read remaps back into the real frame so a single pass handles
	// any requested rotation while filling the viewfinder.
	...
	void primePpaSurface(std::uint16_t *dst, std::size_t bytes)
	{
		...
		for (const PpaPrimedSurface &surface : ppaPrimedSurfaces_) {
			if (surface.dst == dst && surface.bytes == bytes) return;
		}
		std::memset(dst, 0, bytes);
		esp_cache_msync(dst, bytes, ESP_CACHE_MSYNC_FLAG_DIR_C2M);
		ppaPrimedSurfaces_[ppaPrimeCursor_] = {.dst = dst, .bytes = bytes};
		ppaPrimeCursor_ = (ppaPrimeCursor_ + 1) % ppaPrimedSurfaces_.size();
	}

	// Core PPA scale+rotate: the latest frame lands as a dstW x dstH block at
	// (outOffX,outOffY) inside the outPicW x outPicH output picture. PPA validates
	// output with its fixed-point floor(block * scale) rule, so choose the largest
	// centred source crop that still fits and only increase the 1/16 scale when it
	// produces an exact cover. Returns false if the PPA can't service this frame.
	                        ...
	                        int rotCcw = kPreviewRotateCcw)
	{
		...
		auto scaledExtent = [](int block, int numerator) {
			const int whole = numerator / kFrag;
			const int frac = numerator % kFrag;
			return whole * block + (frac * block) / kFrag;
		};
		auto maxBlockForOutput = [](int outputPixels, int numerator) {
			if (outputPixels <= 0 || numerator <= 0) return 0;
			const std::int64_t limit =
				(static_cast<std::int64_t>(outputPixels) + 1) * kFrag - 1;
			const int maxBlock = static_cast<int>(limit / numerator);
			return std::max(1, maxBlock);
		};
		auto makeCropPlan = [&](int numerator) {
			const int maxForOutputW = maxBlockForOutput(dstW, numerator);
			const int maxForOutputH = maxBlockForOutput(dstH, numerator);
			CropPlan plan;
			plan.scaleNum = numerator;
			plan.blockW = std::min(Fw, swap ? maxForOutputH : maxForOutputW);
			plan.blockH = std::min(Fh, swap ? maxForOutputW : maxForOutputH);
			if (plan.blockW <= 0 || plan.blockH <= 0) return plan;
			const int scaledW = scaledExtent(plan.blockW, numerator);
			const int scaledH = scaledExtent(plan.blockH, numerator);
			plan.producedW = swap ? scaledH : scaledW;
			plan.producedH = swap ? scaledW : scaledH;
			return plan;
		};

		CropPlan crop = makeCropPlan(sNum);
		if (crop.producedW != dstW || crop.producedH != dstH) {
			for (int candidate = sNum + 1; candidate <= sNum + 8; ++candidate) {
				CropPlan exact = makeCropPlan(candidate);
				if (exact.blockW <= 0 || exact.blockH <= 0) continue;
				if (exact.producedW == dstW && exact.producedH == dstH) {
					crop = exact;
					break;
				}
			}
		}
		if (crop.blockW <= 0 || crop.blockH <= 0) return false;
		if (crop.producedW > dstW || crop.producedH > dstH) return false;
		sNum = crop.scaleNum;
		const float scale = static_cast<float>(sNum) / kFrag;
		const int blockW = crop.blockW;
		const int blockH = crop.blockH;
		...
		if (++s_ppaN >= 30) {
			ESP_LOGI(kTag, "ppa preview avg=%lldus in=%dx%d block=%dx%d produced=%dx%d out=%dx%d dst=%dx%d scale=%.3f",
			         s_ppaAcc / s_ppaN, Fw, Fh, blockW, blockH, crop.producedW, crop.producedH,
			         outPicW, outPicH, dstW, dstH, scale);
			s_ppaAcc = 0;
			s_ppaN = 0;
		}
		return ppaOk;
	...
	// NativeOverlay direct-present: write straight into the physical panel rect.
	// We compose only the camera mount correction here, not the app/display
	// orientation, so the preview avoids the logical framebuffer rotate+flush path.
	bool presentDirect(int vfX, int vfY, int vfW, int vfH)
	{
		if (!open_ || vfW <= 0 || vfH <= 0) return false;
		// The logical viewfinder rect is mapped to its physical panel bounds only
		// for placement. The camera frame itself is not composed with rotSteps.
		...
		if (gea::platform::display::Display::panelDirectTarget(vfX, vfY, vfW, vfH, &panelBuf, &panelBufW, &panelBufH,
			...
			static bool loggedNativePanel = false;
			if (!loggedNativePanel) {
				loggedNativePanel = true;
				ESP_LOGI(kTag, "camera preview using native panel path: cameraRot=%d panel rotSteps=%d ignored",
				         ((kPreviewRotateCcw % 4) + 4) % 4, rotSteps);
			}
			const std::size_t panelBytes = (static_cast<std::size_t>(panelBufW) * panelBufH * 2 + 127) & ~static_cast<std::size_t>(127);
			bool ok = false;
			withFrame([&](CapturedFrame &frame) {
				const bool fullPanelFlip = flip && pX == 0 && pY == 0 && pW == panelBufW && pH == panelBufH;
				if (fullPanelFlip) primePpaSurface(panelBuf, panelBytes);
				if (!ppaScaleRotateInto(frame, panelBuf, panelBytes, panelBufW, panelBufH, pX, pY, pW, pH, kPreviewRotateCcw)) return;
				if (flip) gea::platform::display::Display::flipPanelToBack(panelBuf);
				recordFrameIfDue(frame);
				ok = true;
			});
			return ok;
		...
		// Native logical path: PPA scale/copy with no camera rotation, then flush the
		// logical rect through the display backend.
	...
	// scales straight into the camera surface or panel back buffers; primed surfaces
	// track which outputs have been blacked so unwritten edge pixels don't show noise.
	ppa_client_handle_t ppaClient_ = nullptr;
	std::size_t ppaOutAlign_ = 64;  // PPA output cache-line alignment (P4 PSRAM = 64 B)
	std::array<PpaPrimedSurface, 4> ppaPrimedSurfaces_{};
	std::size_t ppaPrimeCursor_ = 0;
...
{
	return esp32_p4_m5stack_tab5::camera::CameraDriver::instance().isAvailable();
}

bool Camera::hasPermission()
{
	return esp32_p4_m5stack_tab5::camera::CameraDriver::instance().hasPermission();
}

bool Camera::requestPermission()
{
	return esp32_p4_m5stack_tab5::camera::CameraDriver::instance().requestPermission();
}

bool Camera::open(const std::string &facingHint, int preferredWidth, int preferredHeight)
{
	return esp32_p4_m5stack_tab5::camera::CameraDriver::instance().open(facingHint, preferredWidth, preferredHeight);
}

void Camera::close()
{
	esp32_p4_m5stack_tab5::camera::CameraDriver::instance().close();
}

bool Camera::isOpen()
{
	return esp32_p4_m5stack_tab5::camera::CameraDriver::instance().isOpen();
}

int Camera::width()
{
	return esp32_p4_m5stack_tab5::camera::CameraDriver::instance().width();
}

int Camera::height()
{
	return esp32_p4_m5stack_tab5::camera::CameraDriver::instance().height();
}

int Camera::orientation()
{
	return esp32_p4_m5stack_tab5::camera::CameraDriver::instance().orientation();
}

Facing Camera::currentFacing()
{
	return esp32_p4_m5stack_tab5::camera::CameraDriver::instance().currentFacing();
}

std::string Camera::currentFacingString()
{
	return esp32_p4_m5stack_tab5::camera::CameraDriver::instance().currentFacingString();
}

int Camera::deviceCount()
{
	return esp32_p4_m5stack_tab5::camera::CameraDriver::instance().deviceCount();
}

DeviceInfo Camera::deviceAt(int index)
{
	return esp32_p4_m5stack_tab5::camera::CameraDriver::instance().deviceAt(index);
}

void Camera::drawPreview(int x, int y, int destW, int destH)
{
	esp32_p4_m5stack_tab5::camera::CameraDriver::instance().drawPreview(x, y, destW, destH);
}

int Camera::capture(bool mirror)
{
	return esp32_p4_m5stack_tab5::camera::CameraDriver::instance().capture(mirror);
}

void Camera::setFlash(const std::string &mode)
{
	esp32_p4_m5stack_tab5::camera::CameraDriver::instance().setFlash(mode);
}

void Camera::setZoom(double factor)
{
	esp32_p4_m5stack_tab5::camera::CameraDriver::instance().setZoom(factor);
}

void Camera::setMirror(bool mirror)
{
	esp32_p4_m5stack_tab5::camera::CameraDriver::instance().setMirror(mirror);
}

int Camera::previewMode()
{
	return esp32_p4_m5stack_tab5::camera::CameraDriver::instance().previewMode();
}

bool Camera::fillPreview(std::uint16_t *dst, int dstWidth, int dstHeight, int fit, bool mirror)
{
	return esp32_p4_m5stack_tab5::camera::CameraDriver::instance().fillPreview(dst, dstWidth, dstHeight, fit, mirror);
}

void Camera::positionPreviewLayer(int x, int y, int width, int height)
{
	esp32_p4_m5stack_tab5::camera::CameraDriver::instance().positionPreviewLayer(x, y, width, height);
}

void Camera::hidePreviewLayer()
{
	esp32_p4_m5stack_tab5::camera::CameraDriver::instance().hidePreviewLayer();
}

void Camera::presentNativeOverlay()
{
	esp32_p4_m5stack_tab5::camera::CameraDriver::instance().presentNativeOverlay();
}

bool Camera::startRecording(const std::string &path, double fps)
{
	return esp32_p4_m5stack_tab5::camera::CameraDriver::instance().startRecording(path, fps);
}

double Camera::stopRecording()
{
	return esp32_p4_m5stack_tab5::camera::CameraDriver::instance().stopRecording();
}

bool Camera::isRecording()
{
	return esp32_p4_m5stack_tab5::camera::CameraDriver::instance().isRecording();
}

void Camera::setExposure(const std::string &mode, double bias, double iso, double durationMs)
{
	esp32_p4_m5stack_tab5::camera::CameraDriver::instance().setExposure(mode, bias, iso, durationMs);
}

void Camera::setWhiteBalance(const std::string &mode, double temperatureK, double tint)
{
	esp32_p4_m5stack_tab5::camera::CameraDriver::instance().setWhiteBalance(mode, temperatureK, tint);
}

void Camera::setFocus(const std::string &mode, double pointX, double pointY)
{
	esp32_p4_m5stack_tab5::camera::CameraDriver::instance().setFocus(mode, pointX, pointY);
}

void Camera::setTorch(const std::string &mode, double level)
{
	esp32_p4_m5stack_tab5::camera::CameraDriver::instance().setTorch(mode, level);
}
...
{
	using gea::platform::esp32_p4_m5stack_tab5::camera::CameraDriver;
