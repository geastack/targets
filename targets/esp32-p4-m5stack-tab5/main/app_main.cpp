...
// Registers the platform camera backend (host/camera.cpp) with the runtime's
// board-independent <camera> preview seam. Forward-declared here for the same
// reason as wifi::registerDriver() above.
namespace gea::framework::camera {
void registerCameraSurface();
}

// Wires the microSD card as the persistent file-cache backend (sdcard_mount.cpp).
// Forward-declared for the same reason as the registrations above.
...
extern "C" void app_main(void)
{
	...
	// Wire the microSD card as the persistent file-cache backend (used by the
	// runtime image host's loadFile/writeFile — e.g. the maps OSM tile cache).
	// This call also forces sdcard_mount.cpp into the link.
	gea::platform::storage::installSdCardFileCache();

	// Wire the platform camera backend to the runtime preview seam so a
	// <camera> element / the Camera facade can stream. No hardware is touched
	// until the camera is actually opened.
