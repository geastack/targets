...
constexpr char kTag[] = "lilygo_t5_touch";
...
public:
	...
	bool begin()
	{
		...
		// Touch fully deferred on this board revision: the GT911 bus/pins aren't
		// known yet, and probing a floating I2C bus blocks the GT911 config read
		// (hangs the runtime). Skip touch entirely for now — the frame loop and
		// hardware button still work. Remove this early-out once the real touch
		// bus is identified.
		ESP_LOGW(kTag, "touch probe skipped (deferred on this board revision)");
		return true;
		auto bus = gea::platform::i2c::Bus::primary();
		// Touch is optional on this board (the GT911 may be unpopulated, or need a
		// reset/power detail we haven't wired yet). Report success either way so
		// the runtime does not abort in TouchRuntime::start(); the frame loop and
		// hardware button still work. Real GT911 bring-up is a follow-up.
		if (!bus.available()) {
			ESP_LOGW(kTag, "I2C bus unavailable; running without touch");
			return true;
		}
		...
		if (err != ESP_OK) {
			ESP_LOGW(kTag, "GT911 not found at 0x14 or 0x5d; running without touch");
			return true;
		}
		...
		xTaskCreatePinnedToCore(touchTaskEntry, "lilygo_t5_touch", 3072, this, 7, &touchTask_, 0);
		return true;
	}

	void poll(int nowMs)
	{
		gea::platform::lilygo_t5::buttons::poll(nowMs);
