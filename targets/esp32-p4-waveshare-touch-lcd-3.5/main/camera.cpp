...
// The OV5647 module on this 3.5" board is mounted the opposite way from the 7"
// (and the 3.5" rotates to landscape at the panel MADCTL, not by rotating the
// framebuffer), so the 7"'s rot=1 lands the scene a half-turn off (upside-down).
// rot=3 (three quarter-turns CCW) brings it upright. (Confirmed on-device.)
constexpr int kPreviewRotateCcw = 3;
...
public:
	...
	void close()
	{
		// Stop the preview pump first, while frames still flow (its blocking
		// DQBUF needs the stream alive to return and observe the stop flag).
		if (previewTask_) {
			previewTaskStop_.store(true);
			for (int i = 0; i < 200 && previewTask_; ++i) vTaskDelay(pdMS_TO_TICKS(5));
		}
		if (fd_ >= 0) {
	...
	{
		const bool changed = x != vfX_ || y != vfY_ || width != vfW_ || height != vfH_;
		const int oldX = vfX_, oldY = vfY_, oldW = vfW_, oldH = vfH_;
		vfX_ = x;
		vfY_ = y;
		vfW_ = width;
		vfH_ = height;
		if (changed) onViewfinderGeometryChanged(oldX, oldY, oldW, oldH);
	}
	void hidePreviewLayer()
	{
		const bool changed = vfW_ != 0 || vfH_ != 0;
		const int oldX = vfX_, oldY = vfY_, oldW = vfW_, oldH = vfH_;
		vfW_ = 0;
		vfH_ = 0;
		if (changed) onViewfinderGeometryChanged(oldX, oldY, oldW, oldH);
	...
	// Viewfinder geometry changed (layout switch, e.g. fullscreen <-> inset).
	// Two stale-geometry presents could otherwise land AFTER the renderer
	// repaints the freed region — one mid-composition in the preview pump
	// (killed by the epoch check before its submit) and one already streaming
	// in the panel-push worker (drained here). Order matters:
	//  1. bump the epoch, so no NEW submit can pass the check;
	//  2. take/release vfSubmitLock_, which waits out a pump submit that
	//     passed the check BEFORE the bump but was still blocked inside
	//     presentPanelRectAsync's back-pressure wait (without this, that
	//     submit queues its stale push after the drain — exactly the
	//     one-dead-frame wipe seen on device);
	//  3. drain whatever ended up queued/streaming.
	// The renderer's repaint runs after this returns, so nothing old wipes it.
	void onViewfinderGeometryChanged(int oldX, int oldY, int oldW, int oldH)
	{
		vfEpoch_.fetch_add(1, std::memory_order_acq_rel);
		if (vfSubmitLock_) {
			xSemaphoreTake(vfSubmitLock_, portMAX_DELAY);
			xSemaphoreGive(vfSubmitLock_);
		}
		gea::platform::esp32_p4_waveshare::display_native::waitPanelPushIdle();
		// The logical framebuffer's camera region is never written by the
		// panel-native present, so it still holds a DEAD frame from whenever it
		// was last touched. The renderer's repaint after a layout switch
		// flushes the framebuffer — including that stale region — which wiped
		// the fresh chrome with one old full-screen camera frame and left the
		// SCREEN showing a layout the TREE no longer had (taps then landed on
		// widgets that weren't visually there). Black both rects out so
		// repaints show black until the next live present covers the new rect.
		namespace display_native = gea::platform::esp32_p4_waveshare::display_native;
		if (oldW > 0 && oldH > 0) display_native::clearLogicalRect(oldX, oldY, oldW, oldH);
		if (vfW_ > 0 && vfH_ > 0) display_native::clearLogicalRect(vfX_, vfY_, vfW_, vfH_);
	}
	void presentNativeOverlay()
	{
		if (!open_ || vfW_ <= 0 || vfH_ <= 0) return;
		// The dedicated pump presents at sensor cadence; this per-UI-frame call
		// only ensures it exists (and falls back to render-paced presents if
		// the task can't start).
		if (!ensurePreviewTask())
			presentDirect(vfX_, vfY_, vfW_, vfH_, vfEpoch_.load(std::memory_order_acquire));
	}
...
private:
	...
	bool queryAndConfigureFormat(int preferredWidth, int preferredHeight)
	{
		...
		// (don't override width/height — the ISP fixes them to the sensor mode).
	...
	// rotateCcw is the quarter-turn count applied while scaling (the logical
	// preview uses kPreviewRotateCcw; the panel-native present composes the
	// extra logical->panel turn on top). byteSwapOut makes the PPA emit the
	// panel's MSB-first RGB565 so no CPU byteswap pass is needed downstream.
	bool ppaScaleRotateInto(const CapturedFrame &frame, std::uint16_t *outBuf, std::size_t outBytes,
	                        ...
	                        int rotateCcw = kPreviewRotateCcw, bool byteSwapOut = false)
	{
		...
		cfg.scale_x = scale;
		cfg.scale_y = scale;
		// byte_swap swaps each RGB565 pixel's bytes on the way through; for a
		// 565->565 pass-through that lands the panel's MSB-first byte order in
		// the output with no CPU involvement.
		cfg.byte_swap = byteSwapOut;
		cfg.mode = PPA_TRANS_MODE_BLOCKING;
		return ppa_do_scale_rotate_mirror(ppaClient_, &cfg) == ESP_OK;
	}
	...
	// Dedicated preview pump: blocking DQBUF (sensor cadence) → PPA → vsync-
	// gated push, decoupled from the UI frame loop. Keeping the consumer
	// faster than the sensor (~36 ms of pipeline work vs ~50 ms frames) means
	// the V4L2 queue stays EMPTY, so frames present in strict capture order
	// with no queued-frame latency. When the preview consumed frames at the
	// slower UI-frame-loop pace instead, the queue filled within seconds and
	// the driver recycled buffers underneath the FIFO — dequeued content
	// stopped being monotonic and on-screen motion stepped BACKWARD a frame
	// every couple of seconds ("two frames forward, one back", user-visible).
	// This also takes the ~50 ms camera present cost out of the UI frame loop.
	bool ensurePreviewTask()
	{
		if (previewTask_) return true;
		if (!vfSubmitLock_) vfSubmitLock_ = xSemaphoreCreateMutex();
		if (!vfSubmitLock_) return false;
		previewTaskStop_.store(false);
		if (xTaskCreatePinnedToCore(&CameraDriver::previewTaskEntry, "cam_preview", 8192, this, 4,
		                            &previewTask_, tskNO_AFFINITY) != pdPASS) {
			previewTask_ = nullptr;
			return false;
		}
		return true;
	}
	...
	{
		auto *self = static_cast<CameraDriver *>(arg);
		for (;;) {
			if (self->previewTaskStop_.load(std::memory_order_relaxed) || !self->open_) break;
			// Snapshot the geometry epoch BEFORE the rect: if the layout
			// switches mid-iteration, the pre-submit epoch check drops this
			// frame instead of pushing stale-geometry pixels over the new UI.
			const std::uint32_t epoch = self->vfEpoch_.load(std::memory_order_acquire);
			const int x = self->vfX_, y = self->vfY_, w = self->vfW_, h = self->vfH_;
			if (w <= 0 || h <= 0) {
				vTaskDelay(pdMS_TO_TICKS(10));
				continue;
			}
			// presentDirect paces on DQBUF; back off briefly when it can't run
			// (capture op holds opLock_, stream paused, stale epoch, ...) so
			// this loop never spins hot.
			if (!self->presentDirect(x, y, w, h, epoch)) vTaskDelay(pdMS_TO_TICKS(5));
		}
		self->previewTask_ = nullptr;
		vTaskDelete(nullptr);
	}
	...
	//
	// On this board's landscape orientations the panel is portrait-addressed
	// (the ST7796 scans physical portrait lines; see display.cpp), so the
	// camera takes the panel-native fast path instead: the PPA composes the
	// sensor frame DIRECTLY into a panel-ordered, pre-byteswapped PSRAM buffer
	// (rotation + byteswap in hardware) and one SPI DMA pushes it in scan
	// order — the stock Waveshare demo's structure, zero CPU pixel work.
	// Note: in that mode the logical framebuffer's camera rect is not written
	// (GEADEV screenshots show the UI but a stale camera region).
	bool presentDirect(int vfX, int vfY, int vfW, int vfH, std::uint32_t epoch)
	{
		if (!open_ || vfW <= 0 || vfH <= 0) return false;
		namespace display_native = gea::platform::esp32_p4_waveshare::display_native;
		int px = 0, py = 0, pw = 0, ph = 0, extraRot = 0;
		if (display_native::panelMapLogicalRect(vfX, vfY, vfW, vfH, &px, &py, &pw, &ph, &extraRot)) {
			if (!panelBufs_[0]) {
				panelBufBytes_ = (static_cast<std::size_t>(gea::platform::display::kNativeWidth) *
				                      gea::platform::display::kNativeHeight * sizeof(std::uint16_t) +
				                  127) &
				                 ~static_cast<std::size_t>(127);
				for (auto &buf : panelBufs_) {
					buf = static_cast<std::uint16_t *>(heap_caps_aligned_alloc(
						128, panelBufBytes_, MALLOC_CAP_SPIRAM | MALLOC_CAP_DMA | MALLOC_CAP_8BIT));
					if (!buf) {
						ESP_LOGE(kTag, "panel-native camera buffer alloc failed");
						break;
					}
					// Prime once: the PPA's exact-fit math covers all but a few
					// scale-quantisation edge pixels, which stay black from here on.
					std::memset(buf, 0, panelBufBytes_);
					esp_cache_msync(buf, panelBufBytes_, ESP_CACHE_MSYNC_FLAG_DIR_C2M);
				}
			}
			if (!panelBufs_[0] || !panelBufs_[1]) return false;
			std::uint16_t *target = panelBufs_[panelBufCur_];
			panelBufCur_ ^= 1;
			bool ok = false;
			withFrame([&](CapturedFrame &frame) {
				// Plain little-endian output: the PPA's byte_swap flag scrambles
				// colors whenever the SRM *downscales* (the filter averages the
				// swapped channel bits; only pure-duplication upscales survive —
				// verified on-device at 1x/2x vs 3x zoom). The panel push does
				// the byteswap linearly while bouncing through its staging.
				const std::int64_t t0 = esp_timer_get_time();
				if (!ppaScaleRotateInto(frame, target, panelBufBytes_, pw, ph, 0, 0, pw, ph,
				                        (kPreviewRotateCcw + extraRot) % 4))
					return;
				// Geometry changed while this frame composed (layout switch):
				// drop it rather than push old-rect pixels over the freshly
				// repainted UI. The buffer is requeued by withFrame as usual.
				// The check and the submit hold vfSubmitLock_ so a geometry
				// change can't slip between them (the submit may block tens of
				// ms in back-pressure; onViewfinderGeometryChanged waits the
				// lock out before declaring the pipeline clean). Created in
				// ensurePreviewTask, before the pump exists.
				if (vfSubmitLock_) xSemaphoreTake(vfSubmitLock_, portMAX_DELAY);
				if (vfEpoch_.load(std::memory_order_acquire) != epoch) {
					if (vfSubmitLock_) xSemaphoreGive(vfSubmitLock_);
					return;
				}
				const std::int64_t t1 = esp_timer_get_time();
				// Async: the worker pushes `target` over SPI while the NEXT frame's
				// PPA runs into the other buffer — the push (~34 ms) and the PPA
				// (~23 ms) overlap instead of serializing.
				const bool submitted = display_native::presentPanelRectAsync(target, px, py, pw, ph);
				if (vfSubmitLock_) xSemaphoreGive(vfSubmitLock_);
				if (!submitted) return;
				const std::int64_t t2 = esp_timer_get_time();
				// Periodic preview perf (mirrors fillPreview's): present rate plus
				// the PPA scale vs push-submit (back-pressure) split.
				{
					static int s_n = 0;
					static std::int64_t s_ppaUs = 0, s_pushUs = 0, s_winStartUs = 0;
					static std::int64_t s_lastT2 = 0, s_gapMinUs = 0, s_gapMaxUs = 0;
					if (s_winStartUs == 0) s_winStartUs = t0;
					s_ppaUs += t1 - t0;
					s_pushUs += t2 - t1;
					// Present-to-present gap spread: with the 3-scan vsync grid the
					// min and max should both sit at ~50 ms; a mixed 33/66 spread
					// means the cadence lock slipped (judder).
					if (s_lastT2 != 0) {
						const std::int64_t gap = t2 - s_lastT2;
						if (s_gapMinUs == 0 || gap < s_gapMinUs) s_gapMinUs = gap;
						if (gap > s_gapMaxUs) s_gapMaxUs = gap;
					}
					s_lastT2 = t2;
					if (++s_n >= 300) {
						ESP_LOGI(kTag,
						         "present perf: %.1f/s ppa=%lldus wait=%lldus scan=%lldus gap=%lld..%lldus (avg/300)",
						         s_n * 1e6 / (t2 - s_winStartUs), (long long)(s_ppaUs / s_n),
						         (long long)(s_pushUs / s_n),
						         (long long)display_native::panelScanPeriodUs(),
						         (long long)s_gapMinUs, (long long)s_gapMaxUs);
						s_n = 0;
						s_ppaUs = s_pushUs = 0;
						s_winStartUs = t2;
						s_gapMinUs = s_gapMaxUs = 0;
					}
				}
				recordFrameIfDue(frame);
				ok = true;
		...
		auto *fb = const_cast<std::uint16_t *>(canvas->pixels());
		...
		withFrame([&](CapturedFrame &frame) {
			...
			if (gea::platform::display::Display::framebufferIsPanelDirect()) {
				// canvas() IS the panel scanout buffer (portrait-native, no rotation).
				// The PPA wrote it via DMA and the DPI panel refreshes it continuously,
				// so there is nothing else to do — no rotate-flush, no CPU cache work
				// (the camera owns this rect; the renderer never writes it).
			} else {
				// Logical framebuffer (rotated orientation): invalidate the rows (the
				// landscape stride*2 is 128-aligned) so the rotate-flush can't write
				// back stale UI cache over the camera, then flush + rotate the rect.
	...
	// Panel-native present buffers (landscape orientations): the PPA renders the
	// camera frame panel-ordered into one while the push worker streams the
	// other to the panel (presentPanelRectAsync back-pressures to one in flight).
	std::array<std::uint16_t *, 2> panelBufs_{};
	int panelBufCur_ = 0;
	std::size_t panelBufBytes_ = 0;
	// NativeOverlay viewfinder rect (logical framebuffer px), set by the renderer.
	int vfX_ = 0, vfY_ = 0, vfW_ = 0, vfH_ = 0;
	// Bumped on every viewfinder geometry change; presents composed under an
	// older epoch are dropped before submit (see onViewfinderGeometryChanged).
	std::atomic<std::uint32_t> vfEpoch_{0};
	// Held across the pump's epoch-check + submit so a geometry change can't
	// slip between them (created lazily by the pump).
	SemaphoreHandle_t vfSubmitLock_ = nullptr;
	// Dedicated preview pump (see ensurePreviewTask).
	TaskHandle_t previewTask_ = nullptr;
	std::atomic<bool> previewTaskStop_{false};
